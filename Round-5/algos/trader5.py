"""
trader.py — IMC Prosperity Round 5 (v4 — combined best-of-both)
================================================================
Built by merging the new best algo (568628, +19.6k) with our v3 (+6.6k).

FROM NEW ALGO (adopted wholesale):
  - _imbalance_mm: book-imbalance-aware MM with EMA trend + RSI bias_score
  - _advisor_nudges / SNIPER_RULES: event-driven lead-lag on abnormal moves
  - UV_VISOR_YELLOW → RED + MAGENTA lead-lag
  - _risk_scale_product: dynamic per-product size scaling on recent vol
  - _signal_multiplier: proportional sizing from signal strength
  - Expanded MM_PRODUCTS (12 products vs our 4)
  - group_avg_corr → widen spread when group is trending together

FROM OUR ALGO (kept because they outperform or complement):
  - SLEEP_POD_SUEDE in pack_nav (+1099 vs their 0)
  - MICROCHIP_SQUARE in MM_PRODUCTS (+983 vs their -65)
  - GALAXY_SOUNDS_BLACK_HOLES + SOLAR_FLAMES in group_residual (+521, +340)
  - Reduced windows (20 vs 32) for faster warmup

PRODUCT EXCLUSION LIST (flat position, never trade):
  Confirmed losers across multiple backtests:
  ...
  ROBOT_DISHES, PANEL_1X2, PANEL_2X2, GALAXY_SOUNDS_PLANETARY_RINGS,
  MICROCHIP_OVAL, MICROCHIP_RECTANGLE, ROBOT_LAUNDRY (lag signal only),
  SLEEP_POD_LAMB_WOOL, PANEL_1X4, ROBOT_IRONING
"""
...
class Trader:
    ...
    COHESIVE_CATEGORIES   = {"MICROCHIP", "TRANSLATOR", "UV", "PANEL"}   # PANEL moves cohesively like TRANSLATOR

    # ── MM products — pure spread capture, active from tick 0 ────────────
    # Adopted from new algo (12 products). MICROCHIP_SQUARE added back (we earn +983).
    MM_PRODUCTS = {
        "MICROCHIP_CIRCLE",
        "MICROCHIP_SQUARE",
        "MICROCHIP_TRIANGLE",
        ...
        "PANEL_2X4",
        "ROBOT_VACUUMING",
        "ROBOT_IRONING",   # inelastic bot pinned at stable price — provide liquidity 1 tick ahead
        "ROBOT_DISHES",    # same — deterministic bot, very tight 2-tick spread
    }

    # ── Sniper / advisor rules (NEW from 568628) ─────────────────────────
    # Leader makes an abnormal move → nudge follower before it catches up
    SNIPER_RULES = {
    ...
    # ── Lag rule (from v3, confirmed working) ────────────────────────────
    LAG_RULES = {
        "ROBOT_LAUNDRY": {
    ...
    # OXYGEN_SHAKE: framework says EV_BREATH leads MINT, but ONLY trade peg-break events.
    # Spread is ~12 ticks so we need high threshold to ensure alpha > spread cost.
    SHAKE_SNIPER = {
        "leader":    "OXYGEN_SHAKE_EVENING_BREATH",
        "follower":  "OXYGEN_SHAKE_MINT",
        "sign":       1,
        "move_z":     2.50,   # very high threshold — must be a genuine peg break
        "gap_z":      1.80,
        "size":       1,
    }

    # ── Pack NAV (sleep pod residuals) ───────────────────────────────────
    # Added SLEEP_POD_SUEDE (our unique winner: +1099)
    PACK_NAV_PRODUCTS = {
    ...
    # ── Regression (unchanged — works well) ─────────────────────────────
    REGRESSION_PRODUCTS = {
        "TRANSLATOR_ASTRO_BLACK": {
    ...
    # ── Mean reversion (oxygen shake chocolate) ──────────────────────────
    MR_CONFIG = {
        "OXYGEN_SHAKE_CHOCOLATE": {"window": 26, "entry": 1.70, "exit": 0.35, "size": 2},
    }

    # ── Stress monitor products ──────────────────────────────────────────
    STRESS_PRODUCTS = [
    ...
    # ── Galaxy residual signal (our unique winners) ──────────────────────
    # Only the 2 products confirmed positive across backtests
    GALAXY_RESIDUAL = {
        "GALAXY_SOUNDS_BLACK_HOLES":  {"group": GALAXY, "window": 20, "entry": 1.65, "exit": 0.40, "size": 2},
        "GALAXY_SOUNDS_SOLAR_FLAMES": {"group": GALAXY, "window": 20, "entry": 1.65, "exit": 0.40, "size": 2},
    }

    # ── Snack pair signal windows reduced 36→24 for faster warmup ────────
    SNACK_WINDOW = 24
    ...
    def run(self, state: TradingState):
        ...
        auxiliary.update(self._mean_reversion_targets(history, sticky))
        ...
        # ── Merge targets (priority: structural > auxiliary > nudges) ─────
        final: Dict[str, int] = {}
        ...
        for product, depth in state.order_depths.items():
            ...
            if product in final and target != 0:
                ...
                # No signal, hold inventory → unwind passively to flat
    ...
    def _mean_reversion_targets(self, history, sticky):
        state   = sticky.setdefault("mr", {})
        targets = {}
        for product, cfg in self.MR_CONFIG.items():
            z    = self._zscore(history.get(product, []), cfg["window"])
            prev = state.get(product, 0)
            dirn = self._sticky_direction(prev, -z if z is not None else None, cfg["entry"], cfg["exit"])
            state[product] = dirn
            strength   = self._signal_multiplier(z, cfg["entry"])
    ...
    def _galaxy_residual_targets(self, history, sticky):
        """Our unique winners — keep only confirmed positive products."""
        ...
        for product, cfg in self.GALAXY_RESIDUAL.items():
            ...
            state[product] = dirn
            risk_scale = self._risk_scale_product(history, product)
            size       = int(np.clip(round(cfg["size"] * risk_scale), 1, 3))
            targets[product] = dirn * size
        return targets
    ...
    def _lagger_targets(self, state_trading, history, mids):
        ...
        for leader, cfg in self.LAG_RULES.items():
            ...
            # Economic floor: expected alpha must exceed spread cost
            # Expected convergence ~ gap_z * historical_std, must beat bid-ask spread
            f_depth = state_trading.order_depths.get(follower)
            if f_depth and f_depth.buy_orders and f_depth.sell_orders:
                f_spread = min(f_depth.sell_orders) - max(f_depth.buy_orders)
                f_series = history.get(follower, [])
                if len(f_series) >= 5:
                    recent_std = float(np.std(np.diff(np.array(f_series[-10:], dtype=float)))) if len(f_series) >= 10 else 1.0
                    expected_alpha = abs(gap) * recent_std
                    if expected_alpha < f_spread * 0.6:  # need 60% of spread in expected alpha
                        continue
        ...
        # OXYGEN_SHAKE peg-break sniper (EV_BREATH → MINT)
        cfg_s  = self.SHAKE_SNIPER
        leader_s, follower_s = cfg_s["leader"], cfg_s["follower"]
        if leader_s in mids and follower_s in mids:
            lz_s = self._last_move_z(history.get(leader_s, []), 20)
            fz_s = self._last_move_z(history.get(follower_s, []), 20)
            if lz_s is not None and fz_s is not None:
                gap_s = cfg_s["sign"] * lz_s - fz_s
                if abs(lz_s) >= cfg_s["move_z"] and abs(gap_s) >= cfg_s["gap_z"]:
                    # Economic floor: check that spread cost is worth it
                    f_depth_s = state_trading.order_depths.get(follower_s)
                    if f_depth_s and f_depth_s.buy_orders and f_depth_s.sell_orders:
                        sp = min(f_depth_s.sell_orders) - max(f_depth_s.buy_orders)
                        if sp <= 14:  # only enter if spread hasn't blown out
                            dirn_s = 1 if gap_s > 0 else -1
                            nudges[follower_s] = nudges.get(follower_s, 0) + dirn_s * cfg_s["size"]

        return nudges

    def _advisor_nudges(self, history, mids, prev_mids):
        """
        Event-driven lead-lag (from new algo).
        When a leader makes an abnormal move vs its group peers,
        nudge the follower before it catches up.
        """
        nudges: Dict[str, int] = {}
        ...
        for leader, cfg in self.SNIPER_RULES.items():
            ...
            # Skip if reversion contradicts event strongly
            if np.sign(event_vote) == np.sign(reversion_vote) and abs(reversion_vote) > 0.8: continue
            # Skip if follower already moved in expected direction
    ...
    def _imbalance_mm(self, product, position, depth, history, base_clip, widen=0, bias_score=0.0):
        """
        Book-imbalance-aware market making (adopted from new algo 568628).
        Adjusts quote qty asymmetrically based on OBI + EMA trend + RSI bias.
        Skips if spread < 4 or last move was too extreme (|z| > 2.4).
        """
        best_bid = max(depth.buy_orders)  if depth.buy_orders  else None
        best_ask = min(depth.sell_orders) if depth.sell_orders else None
        if best_bid is None or best_ask is None: return []
        ...
        if move_z is not None and abs(move_z) > 2.4: return []
        ...
        # Passive alpha leaning: when bias is very strong AND spread is wide,
        # shift the entire quote 1 tick in the direction of expected move.
        # This is the "shift both bid and ask" the framework prescribes.
        if abs(bias) > 0.95 and spread >= 6:
            shift = 1 if bias > 0 else -1
            bid_price += shift
            ask_price += shift
            # Safety: don't cross
            if bid_price >= ask_price:
