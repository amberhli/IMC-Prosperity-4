"""
trader.py — IMC Prosperity Round 5 (v6 — v5 regression fixes)
================================================================
v4 profit: 22,024 | v5 profit: 19,956 (−2,069) | v6 target: beat v4

v5 introduced 6 fixes. Second backtest (571638.json) proves 4 of them
were net negative. This version keeps only the 2 confirmed winners and
reverts or corrects the 4 losers.

KEPT FROM v5:
  FIX 1 ✓  MICROCHIP_SQUARE out of MM_PRODUCTS (+65 gain, clean win)
  FIX 4 ✓  EOG unwind for snackpack/pebbles at ts≥95,000 (+35 net)

REVERTED FROM v5:
  FIX 5 ✗  TRANSLATOR_ECLIPSE_CHARCOAL re-added to MM_PRODUCTS
            (-212 by removing it; the v4 -850 trough was an isolated
            event — MM spread income outweighs the occasional adverse hit)

  FIX 6 ✗  move_z guard restored 1.8 → 2.4
            (At z=1.8, ~10% of all MM ticks suppressed vs ~3% at 2.4.
            UV_VISOR_ORANGE lost 657, MICROCHIP_TRIANGLE 154,
            TRANSLATOR_VOID_BLUE 204. Guard was too tight.)

CORRECTED FROM v5:
  FIX 2 ~  Galaxy entry thresholds split by product:
            - GALAXY_SOUNDS_SOLAR_FLAMES: keep 2.10 (gained +159 in v5)
            - GALAXY_SOUNDS_BLACK_HOLES: revert to 1.65, size=1
              (raising to 2.10 dropped it from 507→53)
            Both remain size=1 (contains trough without losing alpha)

  FIX 3 ✗  OXYGEN_SHAKE_CHOCOLATE fully removed from MR_CONFIG
            warmup=40 cleared at ts=4000 (mid already -69pts); product
            has a persistent multi-leg downtrend (9324→8992), not MR.
            v4 +17 was luck. v5 lost -400. Remove entirely.
"""
...
class Trader:
    ...
    # ── MM products ────────────────────────────────────────────────────────
    # MICROCHIP_SQUARE excluded (v5 FIX 1 confirmed ✓)
    # TRANSLATOR_ECLIPSE_CHARCOAL re-included (v5 FIX 5 reverted — cost -212)
    MM_PRODUCTS = {
        "MICROCHIP_CIRCLE",
        "MICROCHIP_TRIANGLE",
        "TRANSLATOR_ASTRO_BLACK",
        "TRANSLATOR_ECLIPSE_CHARCOAL",   # restored
